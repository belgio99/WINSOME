...
public class ServerRequestHandler implements Runnable {
   ...
   public void run() {
      ...
      try {
         ...
         if (requestSplit.length < 1 || requestSplit[0].trim().isEmpty()) { // se la richiesta è vuota o contiene solo spazi
            ...
            ServerUtils.sendString(clientChannel, response);
         ...
         if (requestSplit[0].equals("login")) {
            if (requestSplit.length != 3 || requestSplit[1].trim().isEmpty()
               ...
               ServerUtils.sendString(clientChannel, "L'input non è corretto!");
               registerKey();
               return;
            }
            ServerUtils.sendString(clientChannel, login(requestSplit));
            registerKey();
            return;
         }
         ...
         if ((u = ServerManager.getUserLogged(clientChannel)) == null) {
            ServerUtils.sendString(clientChannel, "Utente non loggato!");
            registerKey();
            return;
         }
         ...
         ServerUtils.sendString(clientChannel, response); // invio la risposta al client
         registerKey(); // registro la chiave del client nel selector
      } catch (IOException e) {
   ...
   private String createPost(User u, String clientRequest) throws IOException {
      String[] split = ServerUtils.fixArray(clientRequest); // rimuovo gli spazi iniziali e finali, e gestisco le virgolette
